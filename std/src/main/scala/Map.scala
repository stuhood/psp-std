...
object Fun {
  ...
  def literal[A, B](kvs: (A->B)*): FiniteDom[A, B] = FiniteDom(kvs.map(_._1).toEqualsSet, partial(kvs map (x => x._1 -> x._2) toMap))
  def orElse[A, B](fs: Fun[A, B]*): Fun[A, B]      = if (fs.isEmpty) empty else fs reduceLeft (OrElse(_, _))
}

object ExMap {
  def empty[K, V] : ExMap[K, V]                                   = new Impl(Fun.literal())
  def apply[K, V](keys: ExSet[K], lookup: Fun[K, V]): ExMap[K, V] = new Impl(FiniteDom(keys, lookup))
  def impl[K, V](xs: ExMap[K, V]): Impl[K, V] = xs match {
    case xs: Impl[K, V] => xs
    case _              => new Impl(xs.lookup)
  }

  def fromJava[K, V](xs: jMap[K, V]): ExMap[K, V]         = new Impl(FiniteDom(xs.keySet.toEqualsSet, Fun(xs get _)))
