...
object ExMap {
  ...
  def fromJava[K, V](xs: jMap[K, V]): ExMap[K, V]           = new Impl(xs.keySet.toEqualsSet, Lookup.total(xs get _))
  def fromScala[K, V](xs: scMap[K, V]): ExMap[K, V]         = xs.toMap |> (xs => new Impl(xs.keys.toEqualsSet, Lookup(xs)))

  // def fromGuava[K, V](xs: gMap[K, V]): ExMap[K, V]         = new FromGuava(xs)

  // final class FromGuava[K, V](xs: gMap[K, V]) extends ExMap[K, V] {
  //   val domain: ExSet.FromGuava[K]       = ExSet fromGuava xs.keySet
  //   def apply(x: K): V                   = xs get x
  //   def foreach(f: ToUnit[K -> V]): Unit = domain foreach (k => f(k -> (xs get k)))
  //   def size: Precise                    = domain.size
  // }
  ...
  final class Impl[K, V](domain: ExSet[K], lookup: Lookup[K, V]) extends InMap.InMapImpl[ExSet, K, V](domain, lookup) with ExMap[K, V] {
    ...
    def +(key: K, value: V): This             = newMap(domain + key, lookup.put(key, value)(domain.eqs))
